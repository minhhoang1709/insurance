...
import com.ninelives.insurance.core.service.B2B2CService;
import com.ninelives.insurance.core.service.B2BService;
import com.ninelives.insurance.core.service.CorporateClientService;
...
public class ApiCmsService {
	...
	public void insertCorporateClient(CorporateClientDto corporateClientDto) throws AppException {
		...
		corporateClientService.insertCorporateClient(cc);

	}

	public CorporateClient fetchByCompanyIdAndCorporateId(String companyName, String corporateId) {
		return corporateClientService.selectByCompanyIdAndCorporateId(companyName, corporateId);
	}
	...
	public List<CompanyRegisterDto> getListCompanyRegister() {
		...
		List<CorporateClient> corporateCompany = corporateClientService.fetchAllCorporateClient();
	...
	public CompanyRegisterDto updateCompanyClient(CompanyRegisterDto companyRegisterDto) {
		...
		CorporateClient cc = corporateClientService.getCorporateClientById(id);
		if (cc != null) {
			...
			if (corporateClientService.updateCorporateClient(cc) == 1) {
	...
	public void insertVoucher(VoucherFormDto voucherFormDto) {
		...
		for (int i = 0; i < coverages.length; i++) {
			...
			if (i == 2) {
				productId3 = product.getProductId();
			}
			if (i == 3) {
				productId4 = product.getProductId();
			}
			if (i == 4) {
				productId5 = product.getProductId();
			}
			if (i == 5) {
				productId6 = product.getProductId();
			}
		}
		String pc = b2bService.selectProductAndCoverage(productId1, productId2, productId3, productId4, productId5,
				productId6);
		if (pc != null) {
	...
	public VoucherRegisterDto updateVoucherClient(VoucherUpdateFormDto voucherUpdateFormDto) {
		...
		LocalDate policyStartDate = dateTimeFormatUtil
				.getLocalDateFromString(voucherUpdateFormDto.getPolicyStartDate());
	...
	public List<ClaimManagementDto> getListClaimManagement() {
		...
		for (int i = 0; i < listClaimManagement.size(); i++) {
			String[] claim = listClaimManagement.get(i).substring(1, listClaimManagement.get(i).length() - 1).split(",",
					-1);
			...
			if (listCoverage != null) {
				...
				for (int j = 0; j < listCoverage.size(); j++) {
					...
					sb.append(
							"<input type=\"text\" class=\"form-control m-input\" readonly=\"readonly\"  id=\"claimNameCoverage\" name=\"claimNameCoverage\" value=\""
									+ cov[1].substring(1, cov[1].length() - 1) + "\">");
					sb.append("<input type=\"hidden\"   class=\"claimCoverageId\" name=\"claimCoverageId\" value=\""
							+ cov[0] + "\">");
	...
	public List<ClaimManagementDto> getListClaimManagementByDate(String startDate, String endDate) {
		...
		for (int i = 0; i < listClaimManagement.size(); i++) {
			...
			if (documentDetail != null || documentDetail.isEmpty()) {
				for (int k = 0; k < documentDetail.size(); k++) {
					String[] doc = documentDetail.get(k).substring(1, documentDetail.get(k).length() - 1).split(",");
					documentInfo += "<a href=" + doc[0].replace("\"", "") + ">" + doc[1].replace("\"", "")
							+ "</a><br/>";
	...
	public ClaimStatusDto updateClaimStatus(ClaimStatusDto claimStatusDto) {
		...
		if (policyClaim != null) {
			...
			for (PolicyCoverageItem policyCoverageItem : listPolicyCoverageItem) {
				PolicyClaimCoverage policyClaimCoverage = b2bService.getListPolicyCliemCoverageByClaimId(claimId,
						policyCoverageItem.getCoverageId());
	...
	public List<TransactionReportInsuranceTypeDto> getListTransactionReportInsuranceType() {
		...
		for (int i = 0; i < listTransaction.size(); i++) {
			...
			transactionByInsuranceType
					.setInsuranceTypeName(cov[0].replace("\"", "") == null ? "" : cov[0].replace("\"", ""));
	...
	@SuppressWarnings("static-access")
	public List<TransactionReportInsuranceTypeDto> getListTransactionReportInsuranceTypeByDate(String startDate,
			String endDate) {
	...
	@SuppressWarnings("static-access")
	public void insertB2B2CVoucher(VoucherFormDto voucherFormDto) {
		
		logger.info("Save B2B2C Voucher :<{}>", voucherFormDto.toString());
		LocalDateTime now = LocalDateTime.now();
		String periodId= voucherFormDto.getPeriod();
		List<String> listProduct= new ArrayList<>();
		boolean beneficiary=true;
		...
		Voucher cc =  new Voucher();
	    	...
	    	cc.setPeriodId(periodId);
			cc.setStatus("ACTIVE");
			cc.setVoucherType(VoucherType.B2B2C);
			cc.setCreatedDate(now);
			cc.setInviterRewardCount(null);
			cc.setMaxUse(voucherFormDto.getMaxUse());
			cc.setApproveCnt(0);
			...
			cc.setProductCount(productCount);
			for (int i = 0; i < coverages.length; i++) {
				Product product = b2b2cService.selectProductByCoverageAndPeriod(coverages[i],periodId);
				listProduct.add(product.getProductId()+","+product.getPremi());
			...
			String pc = b2b2cService.selectProductAndCoverage(productId1,productId2,productId3,
			...
			b2b2cService.insertVoucher(cc);
		
		//Insert to Voucher Product
		for (int i = 0; i < listProduct.size(); i++) {
			
			VoucherProduct voucherProduct =  new VoucherProduct();
			Voucher voucher = b2b2cService.selectVoucherByCode(voucherCode, voucherFormDto.getCorporateClientId());
			String[] prod = listProduct.get(i).split(",");
			
			voucherProduct.setVoucherId(voucher.getId());
			voucherProduct.setProductId(prod[0]);
			voucherProduct.setPremi(0);
			b2b2cService.insertVoucherProduct(voucherProduct);
			
		}
	}

	@SuppressWarnings("static-access")
	public VoucherRegisterDto updateB2B2CVoucherClient(VoucherUpdateFormDto voucherUpdateFormDto) {
		...
		Voucher voucher = b2b2cService.selectVoucherById(voucherId);
		if (voucher != null) {
			voucher.setCode(voucherCode);
			voucher.setTitle(voucherUpdateFormDto.getTitle());
			voucher.setSubtitle(voucherUpdateFormDto.getSubtitle());
			voucher.setDescription(voucherUpdateFormDto.getDescription());
			voucher.setMaxUse(voucherUpdateFormDto.getMaxUse());
			voucher.setUseStartDate(useStartDate);
			voucher.setUseEndDate(useEndDate);
			voucher.setPeriodId(periodId);
			...
			String pc = b2b2cService.selectProductAndCoverage(productId1, productId2, productId3, productId4,
					productId5, productId6);
			...
			if (b2b2cService.updateVoucher(voucher) == 1) {

				if (b2b2cService.deleteVoucherProduct(voucherId) > 0) {
					// Insert to Voucher Product
					for (int i = 0; i < listProduct.size(); i++) {
						VoucherProduct voucherProduct = new VoucherProduct();
						String[] prod = listProduct.get(i).split(",");
	...
	@SuppressWarnings("static-access")
	public List<VoucherRegisterDto> getListB2B2CVoucherByDate(String startDate, String endDate) {
		List<VoucherRegisterDto> dtoList = new ArrayList<>();
		String start = dateTimeFormatUtil.getFormatStatDate(startDate);
		String end = dateTimeFormatUtil.getFormatStatDate(endDate);

		List<Voucher> voucher = b2b2cService.fetchB2B2CVoucherByDate(start, end);
	...
	@SuppressWarnings("static-access")
	public List<VoucherRegisterDto> getListB2B2CVoucherRegister() {
		List<VoucherRegisterDto> dtoList = new ArrayList<>();

		List<Voucher> voucher = b2b2cService.fetchB2B2CVoucher();
	...
	public List<FormObjectDto> getListPeriodByInsuranceType(String insuranceType) {
		List<FormObjectDto> dtoList = new ArrayList<>();
		List<String> listObj = b2bService.getListPeriodByInsuranceType(insuranceType);
		for (int i = 0; i < listObj.size(); i++) {
			String[] cov = listObj.get(i).substring(1, listObj.get(i).length() - 1).split(",");
			FormObjectDto obj = new FormObjectDto();
			obj.setCode(cov[0] == null ? "" : cov[0]);
	...
	public List<UserB2bCodeDto> getListUserB2b2cByCode() {
		...
		List<String> listPromoCode = b2b2cService.fetchUserB2b2cByVoucherCode();
		for (String voucher : listPromoCode) {
			String orderTime = "";
			String[] voc = voucher.substring(1, voucher.length() - 1).split(",");
	...
	public List<UserB2bCodeDto> getListUserB2b2cByDate(String startDate, String endDate) {
		List<UserB2bCodeDto> dtoList = new ArrayList<>();
		String start = dateTimeFormatUtil.getFormatStatDate(startDate);
		String end = dateTimeFormatUtil.getFormatStatDate(endDate);

		List<String> listPromoCode = b2b2cService.fetchUserB2b2cByOrderDate(start, end);
		for (String voucher : listPromoCode) {
			String orderTime = "";
			String[] voc = voucher.substring(1, voucher.length() - 1).split(",");
